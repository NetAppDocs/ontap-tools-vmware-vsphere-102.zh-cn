...
+
.. 导航到* tools*>*保护*>*主机集群关系*，单击该集群对应的“NetApp ONTAP”菜单，然后选择*编辑*或
...
. 如果添加了新的ESXi主机、则保护状态将显示为已部分保护。在SnapMirror设置下选择KERBIOS菜单、然后选择*编辑*以设置新添加的ESXi主机的距离。
+
...
. 选择SnapMirror设置下的KEBIOS菜单，然后选择*Delete*。
